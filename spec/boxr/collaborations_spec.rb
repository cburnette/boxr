...
describe 'collaborations operations' do
  it 'invokes collaborations operations' do
    ...
    collaboration = BOX_CLIENT.add_collaboration(@test_folder, { id: @test_user.id, type: :user },
                                                 :viewer_uploader)
    file_collaboration = BOX_CLIENT.add_collaboration(new_file,
                                                      { id: second_test_user.id, type: :user }, :viewer, type: :file)
    group_collaboration = BOX_CLIENT.add_collaboration(@test_folder,
                                                       { id: test_group.id, type: :group }, :editor)
    ...
    expect do
      BOX_CLIENT.add_collaboration(@test_folder, { id: @test_user.id, type: :user },
                                   :invalid_role)
    end.to(raise_error do
             Boxr::BoxrError
           end)
