...
describe 'file metadata operations' do
  it 'invokes file metadata operations' do
    ...
    metadata = BOX_CLIENT.update_metadata(test_file,
                                          [{ op: :replace, path: '/b', value: 'friend' }])
  ...
  it 'invokes folder metadata operations' do
    ...
    metadata = BOX_CLIENT.update_folder_metadata(new_folder,
                                                 { op: :replace, path: '/b', value: 'there' }, 'enterprise', 'test')
    expect(metadata.b).to eq('there')
    metadata = BOX_CLIENT.update_folder_metadata(new_folder,
                                                 [{ op: :replace, path: '/b', value: 'friend' }], 'enterprise', 'test')
    expect(metadata.b).to eq('friend')
