...
describe 'chunked uploads operations' do
  it 'invokes chunked uploads operations' do
    ...
    new_session = BOX_CLIENT.chunked_upload_create_session_new_file(
      "./spec/test_files/#{TEST_LARGE_FILE_NAME}", @test_folder
    )
    ...
    new_file = BOX_CLIENT.chunked_upload_file("./spec/test_files/#{TEST_LARGE_FILE_NAME}",
                                              @test_folder)
    expect(new_file.name).to eq(TEST_LARGE_FILE_NAME)
    test_file = new_file

    puts 'upload new version of a large file in chunks'
    new_version = BOX_CLIENT.chunked_upload_new_version_of_file(
      "./spec/test_files/#{TEST_LARGE_FILE_NAME}", test_file
    )
